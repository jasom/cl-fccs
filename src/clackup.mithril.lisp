...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defun render-mithril (class options element stream)
      (format stream "
<script>
 m.mount(~a,
         {view : function (){
  ...
  (with-db-connection
    `(200
	 ...
	 (render-mithril "CharacterList"
			 `("value" ,(format nil "Immutable.fromJS(~A)"
					   (with-output-to-string (str)
					     (encode-classish
						 ...
						 collect (make-character-summary
					      ...
					      str))))
			 "document.getElementById(\"mithril-content\")"
			 s)
	 )))))
  ...
  (with-db-connection
    (let ((character (get-character id)))
	  ...
	  `(200
	    ...
	    (,(render-page
	       (s)
	       (format s "<script>VIEWONLY = true;</script>")
	       (render-mithril
		"CharacterMenu"
		`("characterId"
		  ,id
		  "defaultValue"
		  ,(format nil "fixupFcCharacter(Immutable.fromJS(~a))"
			   (encode-classish character nil)))
		"document.getElementById(\"mithril-content\")"
		s))))
	  `(404
	    (:content-type "text/html")
...
(defapprule account-rule (and
  ...
  (with-db-connection
    `(200
	 ...
	 (render-mithril
	  "AccountSettings"
	  `("user" ,(json:encode-json-to-string username))
	  "document.getElementById(\"mithril-content\")"
	  s))))))
